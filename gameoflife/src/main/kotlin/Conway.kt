...
object Conway {
    ...
    const val Size = 10 // Size of grid

    val R: Set<Int> = as_Set(1..Size)
    val A: Set<Int> = mk_Set(-1, 0, 1)
    ...
    interface Population : Set1<Point>

    fun around(p: Point) =
        dunion(
            set(A) { x: Int -> set(A) { y: Int -> mk_Point(p.x + x, p.y + y) } }
        ).minus(p)
    // Not sure how to add the post command that around(p).card < 9 ?

    fun neighbourCount(pop: Population, p: Point) =
        mk_Set(
            around(p) inter pop
        ).card
    // Not sure how to add the post command that neighbourCount < 9

    fun newCells(pop: Population) =
        dunion(
            set(pop) { p: Point ->
    ...
    // Needs post command where RESULT inter pop == {}

    fun deadCells(pop: Population) =
        dunion(
            set(pop, filter = { neighbourCount(pop, it) in Survive })
            { it }
        )
    // Needs post command where post RESULT inter pop == RESULT

    fun generation(pop: Population) =
        (pop minus deadCells(pop)) union newCells(pop)

    fun generations(n:Int, pop:Population) =
