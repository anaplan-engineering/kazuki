...
infix fun <D, R, T : Relation<D, R>> T.domRestrictTo(s: Set<D>) = transformSet { it.elements.kotlinFilter { (k, _) -> k in s } }

infix fun <D, R, T : Relation<D, R>> T.drt(s: Set<D>) = domRestrictTo(s)

infix fun <D, R, T : Relation<D, R>> T.rngRestrictTo(s: Set<R>) = transformSet { it.elements.kotlinFilter { (_, v) -> v in s } }

infix fun <D, R, T : Relation<D, R>> T.rrt(s: Set<R>) = rngRestrictTo(s)

infix fun <D, R, T : Relation<D, R>> T.domSubtract(s: Set<D>) = transformSet { it.elements.kotlinFilter { (k, _) -> k !in s } }

infix fun <D, R, T : Relation<D, R>> T.dsub(s: Set<D>) = domSubtract(s)

infix fun <D, R, T : Relation<D, R>> T.rngSubtract(s: Set<R>) = transformSet { it.elements.kotlinFilter { (_, v) -> v !in s } }
